...
def parsing_xl1(item_path,chip_name,test_time):
    ...
    for row in range(1, file_shape[0]):
        if single_item_data.iloc[row, 0] == "TestSettingsData":

            #   临时存放测试数据和测试状态
            temp_data_info = []
            temp_status_info = []

            for col in range(6, 14, 2):
                if single_item_data.iloc[row, col] != 'Na':
                    temp_data_info.append(single_item_data.iloc[row + 2, col - 5])
                    temp_status_info.append(single_item_data.iloc[row + 2, col - 4])
            test_info_data.extend(temp_data_info)
            test_info_status.extend(temp_status_info)
    #取出整行测试条件及spec，插入测试数据和结果到同一个list
    for col in range(2, 14):
        if col != 5:
            test_info_all.append(single_item_data.iloc[15, col])
